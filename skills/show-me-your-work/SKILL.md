...
metadata:
  compatibility: Uses bstack-runtime for optional current-task history and independent review. The TSV helper requires a POSIX shell.
---
...
Before handing back, start one independent read-only reviewer through **bstack-runtime** using the `critic` role when delegation is available. The reviewer reads the audit trail and current-task history, then flags what the user should scrutinize. When delegation is unavailable, perform the audit directly and disclose that independent review was unavailable.
