...
Load **bstack-runtime** and start all explorers through its read-only parallel delegation capability:

- Worker contract: read-only exploration.
- Model role: configured how-explorer role, default `fast-code`.
...
Start a single read-only worker that explores and explains in one pass, or perform the pass directly when delegation is unavailable:

- Model role: configured how-explainer role, default `judgment`.
- Contract: no writes or external mutations.
...
Once all explorers return, start a single read-only worker to synthesize their findings into one coherent explanation, or synthesize directly:
...
After the explanation is complete, start the configured critic panel through **bstack-runtime**, bounded by `panel-size` and `max-parallel`.

For each critic:
- Model role: `critic`, with additional configured semantic roles when the panel is larger than one.
- Contract: read-only architectural review.
