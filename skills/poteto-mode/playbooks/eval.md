...
6. **Verify the chain from host evidence, not self-report.** Use **bstack-runtime** to obtain each candidate's in-scope execution history when available. Never scan unrelated conversations. Check which files and tools the candidate actually used. If the host cannot expose execution history, grade only observable artifacts and mark chain-following unverified.
