...
- **Coordinator (this chat).** Frames, authors briefs, drains the inbox, owns the human report, and makes judgment calls. It never authors or edits program code: conflicted merges, restacks, and code changes are delegated work items. Mechanically landing a verified unit is allowed only when the runtime authorization covers the exact commit, push, and merge operations. Delegate, resume, wait, and inspect status only through capabilities exposed by **bstack-runtime**. State reads and writes go through `scripts/orch/orch.ts` at drain points. The CLI never delegates, waits, or wakes anything.
- **Sub-coordinator.** Durable, one per track, and only when the program exceeds what one coordinator can drain. A track the coordinator can drain itself needs no middle layer. Owns its track's units and boards, authors its workers' briefs, and delegates its own workers and verifiers only when the runtime supports nesting. Rolls up aggregates at wave boundaries. Never forwards raw child reports. Cap in-flight children at the runtime limit and what one drain can process.
- **Worker / verifier.** Place each worker in a local, remote, or isolated environment according to the runtime and the task's filesystem, authentication, simulator, IDE, and control requirements. A worker that cannot read the coordinator's durable store receives the required context inline or through repository paths. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). When model diversity is available, use a different semantic model role for verification than implementation.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so a completion flood cannot wipe pending runtime `ask` state.
...
- Never resume a worker merely to check on it when the host treats resume as execution. Probe read-only through the ledger, `units.tsv`, provider state, pushed branches, and the runtime's status capability. History timestamps alone are not liveness.
...
- For explicit CLI routes, follow the runtime's [recovery rule](../../bstack-runtime/SKILL.md#recover-explicit-executor-failures). Repair recoverable launch or permission mismatches and retry up to three times after the initial attempt, without changing models or resetting the budget on respawn. For native `auto` workers, use the same retry cap: reduce context on cap-hit or oom while preserving required coverage, repair tool errors, and back off on network failures. Stop for genuine access or policy denials. After exhaustion, record the blocker and replan independent work without claiming the failed unit is complete.
...
- Bound your own infra retries the same way you bound a child's, at three retries after the initial attempt. After exhaustion, write a terminal handoff to durable state with what is done, where it lives, and the exact command to resume. Stop the dependent work and continue only independent authorized work.
- After a host restart or context loss, treat worker liveness as unknown until the runtime confirms it. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach durable work by PR and branch rather than transient worker id, recreate missing coordinators from stored briefs, then drain and resume. `orch` replaces a stale lock whose holder pid is gone.
