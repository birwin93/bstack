...
4. Verify each component against its baseline via image diff on the matching surface through a real control capability. A nonzero diff is a fail. Investigate the pixel delta. Iterate per component until the diff is zero, using the runtime's wake capability for unattended work when available.
