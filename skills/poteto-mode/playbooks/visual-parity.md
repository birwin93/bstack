...
4. Verify each component against its baseline via image diff on the matching surface through a real control capability. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Iterate per component until the diff is zero, using the runtime's wake capability for unattended work when available.
5. Run **Opening a PR** per component or safe batch only when the user authorized publication.
