...
1. Locate the prior trail through **bstack-runtime**. Use the current workspace's history capability, a user-supplied task URL, a decision log, or a branch. Never scan unrelated conversations. Read the overview and last completed messages first, then scan back for decision points. Reduce a long history through a read-only worker when delegation is available.
