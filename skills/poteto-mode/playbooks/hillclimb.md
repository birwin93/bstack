...
5. Loop, one hypothesis per iteration:
   ...
   - Log every accepted or reverted fix. When commits are authorized, use one commit per accepted fix and stage only the files you changed (`git add <files>`, never `-A`).
...
8. If the user authorized publication, run **Opening a PR** with accepted units in the order they landed. Otherwise keep the verified result local.
