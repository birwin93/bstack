...
**Start every multi-step task with a tracked checklist whose first item is to read the Principles section below in full.** Use the host's planning or todo capability when available; otherwise keep the checklist in the working response. The principles ground every trigger here. In your reply, name only principles that shaped a real decision and the choice each changed.
