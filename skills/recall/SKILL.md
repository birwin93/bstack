...
metadata:
  compatibility: Uses bstack-runtime for optional current-workspace history, connected tools, and bounded delegation. Degrades to supplied context and live repository state.
---
...
Load **bstack-runtime** and use only its current-workspace `history` capability. Never guess a host transcript path or scan unrelated projects. When history is unavailable, use the current conversation, host-provided memory, and sources the user explicitly supplies.
...
3. Search the in-scope history. Use bounded read-only delegation with the `fast-code` role when the corpus is large and the runtime exposes delegation; otherwise search serially. Order records by their host-provided timestamps, search the topic before opening full records, and skip worker or eval noise. Each result uses the same schema: topic, user goal, decisions, open threads, struggles and corrections, artifacts, and a stable host-provided citation. Keep raw history out of the main context when summaries are sufficient.
