...
metadata:
  compatibility: Uses bstack-runtime for optional current-task history and bounded delegation. Requires an available skill-authoring workflow to create the final skill.
---
...
This skill orchestrates three others: an inline mining pass, the host's available skill-authoring workflow, and the **unslop** skill. It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively in the repository and personal Agent Skills locations exposed by the current host for `*-mode/SKILL.md` matching the user's handle. If one exists, use the **bstack-runtime** `ask` capability to confirm intent unless the user already said "update my skill" or similar:
...
Load **bstack-runtime** and request the current workspace's in-scope history capability. Never scan unrelated projects or private conversations. If history is unavailable, mine the current conversation and any sources the user explicitly provides.
...
Mining misses intent that hasn't come up yet. Use the runtime `ask` capability and prefer concise structured choices when the host supports them.
...
Use the host's available skill-authoring workflow to author the skill. Placement:
...
- Keep the description narrowly triggered on the user's handle and explicit mode request. Add a host-specific explicit-invocation policy only when the host supports one and the user wants it.
...
Write the approved skill in the requested scope and validate it. Commit, push, or open a pull request only when the user explicitly authorizes those actions.
...
- The host's available skill-authoring workflow.
