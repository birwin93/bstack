...
- **Take a census before the next fix.** Count the imbalance per actor. Record both the size of the imbalance and which actors hold it. Write the census as a rerunnable script per [Build the Lever](../principle-build-the-lever/SKILL.md).
- **Read the skew.** If the same few actors hold most of the imbalance on every run, investigate what assigns them that role and verify that the assignment causes the skew. That assignment is the next "why" per [Fix Root Causes](../principle-fix-root-causes/SKILL.md).
...
- If the census is even across actors, it does not support skew in the measured quantity. Reject a premise that directly predicts unequal counts when reliable counts are equal. Test other mechanisms separately, since equal counts do not establish equal cost or latency.
