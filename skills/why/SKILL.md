...
metadata:
  compatibility: Uses bstack-runtime to discover optional connected tools and bounded read-only delegation. Git history is sufficient for a reduced investigation.
---
...
Load **bstack-runtime** and inspect its `connected-tools` capability map. Use only tools exposed by the current host. Do not inspect undocumented host directories to infer unavailable integrations.
...
Use git and `gh` for source control when available, or investigate the records the user supplied. Report any unavailable source or tool as a coverage gap. For the other six categories, classify using the MCP name, server instructions, tool names, and resource descriptors. If an MCP could fit more than one category, choose the one matching its primary evidence. Record ambiguous cases in the coverage map.

Aim for a complete **coverage map**, not a minimal one. Document the null, don't skip the search.

Start matching investigators through the runtime's bounded parallel delegation capability. Give every investigator a read-only evidence posture plus the minimum connected-tool access needed for its category. When delegation is unavailable, search categories serially.
...
Use one investigator per available evidence category, or investigate the categories serially when delegation is unavailable. Each investigator owns one category.
...
1. **Source control investigator**. Available git history, `gh` for PRs, code comments, tests, and supplied source-control records. Search this category when records are accessible, or report the gap. Best at surfacing *implementation-time rationale captured during review*.
...
Start one synthesizer through **bstack-runtime**, using the `judgment` role and read-only access to the gathered evidence, or synthesize directly when delegation is unavailable. The synthesizer may spot-verify citations through available connected tools but must not write to them.
