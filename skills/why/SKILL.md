...
Start matching investigators through the runtime's bounded parallel delegation capability. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Give every investigator a read-only evidence posture plus the minimum connected-tool access needed for its category. When delegation is unavailable, search categories serially.
...
Start one synthesizer through **bstack-runtime**, using the `judgment` role and read-only access to the gathered evidence. The synthesizer may spot-verify citations through available connected tools but must not write to them.
